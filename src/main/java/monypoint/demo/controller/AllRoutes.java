...
public class AllRoutes {
    ...
    public String handlePhoneLogin(@RequestParam String phoneNumber, Model model) {
        logger.debug("Processing phone login for: {}", phoneNumber);
        String normalizedPhone = phoneNumber.replaceAll("[^0-9+]", "");
        if (!normalizedPhone.matches("^\\+234[0-9]{10}$")) {
            model.addAttribute("error", "Invalid phone number. Must be +234 followed by 10 digits.");
            logger.warn("Invalid phone number format: {}", normalizedPhone);
        ...
        if (user == null) {
            ...
            logger.warn("Phone number not found: {}", normalizedPhone);
            return "login";
        }
        try {
            String otp = verificationService.sendOtp(normalizedPhone);
            model.addAttribute("phoneNumber", normalizedPhone);
            model.addAttribute("otp", otp);
            logger.info("OTP generated for phone login: {}", normalizedPhone);
        } catch (Exception e) {
            model.addAttribute("error", "Failed to generate OTP. Please try again.");
            logger.error("Error generating OTP for phone {}: {}", normalizedPhone, e.getMessage(), e);
        }
    ...
    public String handlePhoneOtpVerify(@RequestParam String phoneNumber, @RequestParam String otp1,
        ...
        if (verificationService.verifyOtp(phoneNumber, otp)) {
            ...
            if (user != null && userService.isUserAuthenticated(user)) {
                user.setPhoneVerified(true);
                userService.save(user);
                model.addAttribute("phoneUser", user);
                model.addAttribute("phoneNumber", phoneNumber);
                logger.info("Phone OTP verified for: {}", phoneNumber);
                return "redirect:/dashboard";
            }
        }
        model.addAttribute("error", "Invalid OTP.");
        model.addAttribute("phoneNumber", phoneNumber);
        logger.warn("Invalid OTP for phone login: {}", phoneNumber);
        return "login";
    }
